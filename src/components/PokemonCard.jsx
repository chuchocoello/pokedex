...
navigate(`/pokedex/${dataPokemon?.id}`)
    ...
    return (
        <article onClick={handleClickPokemon} className={`pokeCard border-${dataPokemon?.types[0].type.name}`}>
            <section className={`pokeCard__header bg-${dataPokemon?.types[0].type.name}`}></section>
            <section className='pokeCard__content'>
                ...
                <h3 className={`pokeCard__name letter-${dataPokemon?.types[0].type.name}`}>{pokemon.name}</h3>
